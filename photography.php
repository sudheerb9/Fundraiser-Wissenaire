...
if (empty($_SESSION['email'])) {
   ...
   $result=mysqli_fetch_array($q);
        ...
        $sql = "SELECT * FROM events WHERE email = '$email' AND event = 'photography'";
		$r=mysqli_query($conn, $sql);
		$checkrow=mysqli_num_rows($r);
		if ($checkrow == 0) {
		   echo '<script type="text/javascript">alert("You have not registered for Through the lens yet ")</script>';
    ...
    if(isset($_POST['insert'])){
        ...
        if($_FILES['image']['size'] > 1000000){
            echo '<script type="text/javascript">alert("File uploaded exceeds maximum upload size.")</script>';
        }
        else{
            
            $folder = 'uploads/photography/';
            $temp = explode(".", $_FILES["image"]["name"]);
            $newfilename = round(microtime(true)) . '.' . end($temp);
            move_uploaded_file($filetmpname, $folder. $newfilename);
            $query = "INSERT INTO photography (email, filename, caption) VALUES ('".$result['email']."', '".$newfilename."', '".$_POST['caption']."') ";
            if(mysqli_query($conn, $query)){
                 echo '<script type="text/javascript">alert("Image uploaded successfully ")</script>';
            }
            else{
                  echo '<script type="text/javascript">alert("There is a problem uploading the file. ")</script>'; 
            }
        }
    }
        ...
        <ul class="list-unstyled">
          ...
          <li class="active"><a href="#play" aria-expanded="false" data-toggle="collapse"> <i class="icon-windows"></i>Participate </a>
            <ul id="play" class="collapse list-unstyled ">
              <li class="active"><a href="photography.php">Through the lens</a></li>
      ...
      <div class="page-content">
        <div class="page-header">
          <div class="container-fluid">
            <h2 class="h5 no-margin-bottom">Through the lens</h2>
          </div>
        </div>
        <section class="no-padding-top no-padding-bottom">
          <div class="container">
              <p>Well, two photographs will be considered for evaluation. Come back! to upload the second file !</p><br>
              <form method="post" enctype="multipart/form-data">
                 <div class="form-group row">
                    <label class="col-sm-3 form-control-label">Upload Your File Here </label>
                    <div class="col-sm-9">
                      <input type="file" name="image" id="image" />
                    </div>
                 </div><br>
                 <div class="form-group row">
                    <label class="col-sm-3 form-control-label">Caption of the picture</label>
                    <div class="col-sm-9">
           ...
           $('#insert').click(function(){
               ...
               if(image_name== ''){
                   ...
                   if(jQuery.inArray(extension, ['jpg', 'jpeg', 'png']) == -1){
                       alert(' Please Upload jpg or jpeg or png file');
                       $('#image').val('');
                       return false;
                   }
               }
